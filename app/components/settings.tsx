...
export function Settings() {
  ...
  const shouldHideBalanceQuery = useMemo(() => {
    const isOpenAiUrl = accessStore.tencentUrl.includes(TENCENT_BASE_URL);

    return (
      accessStore.hideBalanceQuery ||
      isOpenAiUrl ||
      accessStore.provider === ServiceProvider.Tencent
    );
  }, [
    accessStore.hideBalanceQuery,
  ...
  const soochowLifeAIConfigComponent = accessStore.provider ===
    ServiceProvider.SoochowLife && (
    <>
      <ListItem
        title={Locale.Settings.Access.SoochowLife.Endpoint.Title}
        subTitle={Locale.Settings.Access.SoochowLife.Endpoint.SubTitle}
      >
        <input
          aria-label={Locale.Settings.Access.SoochowLife.Endpoint.Title}
          type="text"
          value={accessStore.soochowLifeUrl}
          placeholder={SOOCHOW_LIFE_BASE_URL}
          onChange={(e) =>
            accessStore.update(
              (access) => (access.soochowLifeUrl = e.currentTarget.value),
            )
          }
        ></input>
      </ListItem>
              ...
              content={
                <AvatarPicker
                  onEmojiClick={(avatar: string) => {
                    updateConfig((config) =>
                      config.avatar == User.Man ? User.Woman : User.Man,
                    );
